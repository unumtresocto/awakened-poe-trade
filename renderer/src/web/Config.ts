...
function upgradeConfig(_config: Config): Config {
  ...
  if (config.configVersion < 19) {
    config.widgets.push({
      ...defaultConfig().widgets.find((w) => w.wmType === "alt-crafting")!,
      wmId: Math.max(0, ...config.widgets.map((_) => _.wmId)) + 1,
      wmZorder: null,
    });
...
function getConfigForHost(): HostConfig {
  ...
  const altCrafting = AppConfig("alt-crafting") as widget.AltCraftingWidget;
  if (altCrafting.hotkey) {
    actions.push({
      shortcut: altCrafting.hotkey,
      action: {
        type: "alt-crafting",
      },
    });
    actions.push({
      shortcut: "Shift + " + altCrafting.hotkey,
      action: {
        type: "alt-crafting",
        stop: true,
      },
